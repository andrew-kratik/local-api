...
exports.start = function (ramlUrl, options) {
    ...
    options.cors = typeof (options.cors) != 'undefined' ? options.cors : false;

    if (options.cors) {
        winston.info("This server is ", "CORS-enabled".red, " for all origins!");
        app.use(cors());
    } else {
        winston.info("CORS disabled, use --cors to enable");
    }
